...
local choice_apreenderitens = {function(source, choice)
    ...
    if user_id then
        if nplayer then
            ...
            local function isBlackItem(item)
                for _, blackItem in ipairs(blackItens) do
                    if blackItem == item then
                        return true
                    end
                end
                return false
            ...
            if inv then
                for k, v in pairs(inv) do
                    if isBlackItem(v["item"]) then
                        local amount = parseInt(v["amount"])
                        if amount > 0 then
                            vRP.tryGetInventoryItem(nuser_id, v["item"], amount, true)
                            vRP.giveInventoryItem(user_id, v["item"], amount, true)
                            print("Item apreendido: " .. v["item"] .. " - Quantidade: " .. amount)
                            print("Item " .. v["item"] .. " transferido para o policial.")
                        end
                    end
                end
            end
            ...
            TriggerClientEvent("Notify", nplayer, "negado", "Seus itens foram apreendidos.", 8000)
            TriggerClientEvent("Notify", source, "sucesso", "Você apreendeu todos os itens.", 8000)
        else
            TriggerClientEvent("Notify", source, "negado", "Nenhum jogador próximo.", 8000)
        end
    end
...
RegisterCommand('apreender', function(source, args, rawCommand)
    local user_id = vRP.getUserId(source)
    if not (vRP.hasPermission(user_id, 'perm.policia') or vRP.hasPermission(user_id, 'admin.permissao')) then
        TriggerClientEvent('Notify', source, "negado", "Apenas policiais podem utilizar este comando.", 6000)
        return
    end

    local nplayer = vRPclient.getNearestPlayer(source, 5)
    local nuser_id = vRP.getUserId(nplayer)
    if not nplayer or not nuser_id then
        TriggerClientEvent('Notify', source, "negado", "Nenhum jogador próximo encontrado.", 6000)
        return
    end
    ...
    local function transferirItens()
        ...
        TriggerClientEvent("Notify", source, "sucesso", "Você apreendeu os itens ilícitos.", 8000)
        TriggerClientEvent("Notify", nplayer, "negado", "Seus itens foram apreendidos.", 8000)
    end
    transferirItens()
end)
...
function RegisterTunnel.reduzirPrisao(reduzir)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        local value = vRP.getUData(parseInt(user_id), "vRP:prisaoS")
        local tempo = json.decode(value) or 0
        if tempo > 0 then

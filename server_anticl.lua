...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        ...
        local datatable = json.decode(data) or {}

        if datatable.health and datatable.health > 0 then
            TriggerClientEvent("setPlayerHealth", source, datatable.health) -- Envia saúde salva ao cliente
        else
            TriggerClientEvent("setPlayerHealth", source, 400) -- Define a vida padrão para 400
        end
    end
end)
...
AddEventHandler("vRP:onLeave", function(user_id, source)
    local playerPed = GetPlayerPed(source)
    local health = GetEntityHealth(playerPed)
